...
if(isset($_POST['add-article'])) {
  ...
  if(!is_dir($uploadDir)){
    mkdir($uploadDir,0777,true); //creer le repertoire s'il n'existe pas
      }
    ...
    if(in_array($extension,$allowedExtensions)){
        ...
        $extension;//Generer un nom de ficier unique

        move_uploaded_file($_FILES['image']['tmp_name'],$imagePath);//deplacer le fichier telecharge

    }else{
        $error = "le fichier telecharger n'est pas valide.";
...
//but du tampon de la page de sortie
ob_start();

// Inclure le layout de la page d'accueil
require_once 'resources/views/admin/articles/add-articles_html.php';
